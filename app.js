...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 5.5 통합 음성 입력 (한 문장으로 여러 항목 자동 입력) ---
    // 결함 원인은 의도적으로 파싱 대상에서 제외 — 오인식 위험이 커서 항상 수동 선택하도록 함.
    function parseIntegratedDefectSpeech(transcript) {
        const result = { component: null, type: null, location: '', sizeText: null, crackWidth: null, crackLength: null, leak: false, progress: false };
        let remaining = transcript;
        const consume = (str) => { remaining = remaining.split(str).join(' '); };

        const widthMatch = transcript.match(/(\d+(?:\.\d+)?)\s*(?:mm|미리|밀리미터|밀리)/);
        const lengthMatch = transcript.match(/(\d+(?:\.\d+)?)\s*(?:m|미터)(?!m)/);
        if (widthMatch) { result.crackWidth = widthMatch[1]; consume(widthMatch[0]); consume('폭'); }
        if (lengthMatch) { result.crackLength = lengthMatch[1]; consume(lengthMatch[0]); consume('길이'); }
        if (result.crackWidth || result.crackLength) {
            result.sizeText = [
                result.crackWidth ? `W=${result.crackWidth}mm` : null,
                result.crackLength ? `L=${result.crackLength}m` : null
            ].filter(Boolean).join(', ');
        }

        if (/누수/.test(transcript)) { result.leak = true; consume('누수'); }
        if (/진행\s*중|진행되고\s*있\S*/.test(transcript)) {
            result.progress = true;
            remaining = remaining.replace(/진행\s*중|진행되고\s*있\S*/g, ' ');
        }

        const componentSelect = document.getElementById('defectComponent');
        if (componentSelect) {
            const options = Array.from(componentSelect.options)
                .filter(o => o.value && o.value !== '__ADD_CUSTOM_COMPONENT__' && o.value !== '기타')
                .sort((a, b) => b.value.length - a.value.length);
            const hit = options.find(o => transcript.includes(o.value));
            if (hit) { result.component = hit.value; consume(hit.value); }
        }

        const typeSelect = document.getElementById('defectType');
        if (typeSelect) {
            const options = Array.from(typeSelect.options)
                .filter(o => o.value && o.value !== '__ADD_CUSTOM__' && o.value !== '기타');
            let best = null, bestTokens = null, bestLen = 0;
            options.forEach(opt => {
                opt.value.split(/[\/\s]+/).filter(Boolean).forEach(tok => {
                    if (tok.length >= 2 && transcript.includes(tok) && tok.length > bestLen) {
                        best = opt.value; bestLen = tok.length;
                        bestTokens = opt.value.split(/[\/\s]+/).filter(Boolean);
                    }
                });
            });
            if (best) { result.type = best; bestTokens.forEach(tok => consume(tok)); }
        }

        result.location = remaining.replace(/\s+/g, ' ').trim();
        return result;
    }
    ...
    function applyIntegratedDefectSpeechResult(transcript) {
        ...
        const componentSelect = document.getElementById('defectComponent');
        if (componentSelect && result.component) {
            componentSelect.value = result.component;
        ...
        const locationField = document.getElementById('defectLocation');
        if (locationField && result.location) {
            locationField.value = locationField.value ? (locationField.value + ' ' + result.location) : result.location;
            filled.push('상세위치');
        } else {
            missed.push('상세위치');
        }

        const sizeField = document.getElementById('defectSize');
        if (sizeField && result.sizeText) {
            sizeField.value = result.sizeText;
            filled.push('규모');
        } else {
            missed.push('규모(수치)');
        }
        const widthField = document.getElementById('defectCrackWidth');
        const lengthField = document.getElementById('defectCrackLength');
        if (widthField && result.crackWidth) widthField.value = result.crackWidth;
        if (lengthField && result.crackLength) lengthField.value = result.crackLength;

        const leakChk = document.getElementById('defectLeakCheck');
        if (leakChk && result.leak) { leakChk.checked = true; filled.push('누수여부'); }
    ...
    window.startIntegratedSpeechRecognition = function(btnElement) {
        ...
        const resetBtn = () => {
            if (btnElement) {
                ...
                btnElement.innerHTML = '<i class="fa-solid fa-microphone-lines"></i> 🎙️ 통합 음성 입력 (한 문장으로 말하기)';
            }
        };
        ...
        try {
            ...
            if (btnElement) {
                btnElement.style.background = '#ef4444';
                btnElement.innerHTML = '<i class="fa-solid fa-microphone"></i> 🔴 듣는 중... (부재/종류/위치/규모를 한 문장으로 말씀해 주세요)';
            }
            ...
            recognition.onresult = function(event) {
                const transcript = event.results[0][0].transcript;
                if (transcript) applyIntegratedDefectSpeechResult(transcript);
            };
    ...
    const btnSpeechIntegrated = document.getElementById('btnSpeechIntegrated');
    if (btnSpeechIntegrated) {
        btnSpeechIntegrated.addEventListener('click', () => window.startIntegratedSpeechRecognition(btnSpeechIntegrated));
    }
